...
class AppDraw {
	...
	if (this._paper) {
	    ...
	    var app = this.app;
	    this._paper = paper.setup([ app.$3D.element.clientWidth, app.$3D.element.clientHeight ]);
	    return this._paper
	}
    }
	...
	return {
	    mousedown: event => {
		// console.log(event);
		// console.log(draw.getIntersectingFace(event, true));
	    },
	    mouseup: event => {},
	    mousemove: event => {},
	}
...
AppDraw.prototype.shapeToCatmullFreehand = function(shape, density){
	...
	geometry.name = 'VectorRayLine_' + Math.floor(Math.random() * 100)
...
AppDraw.prototype.getIntersectingFace = function(p1, all) {
    ...
    var intersects = raycaster.intersectObjects( [ app.mesh ], true );
